...
public class EmployeeDao extends AbstractDao<Employee> implements EmployeeDaoI{
    ...
    public EmployeeDao() {
        super("INSERT INTO employee(idStatus, name, firstName, mail, password, job) VALUES (?, ?, ?, ?, ?, ?)",
         "SELECT * FROM employee",
          "SELECT * FROM employee WHERE id = ?",
           "UPDATE employee SET idStatus = ?, name = ?, firstName = ?, mail = ?, password = ?, job = ? WHERE id = ?",
    ...
    //Méthode permettant de remplacer les parenthèse dans la requête SQL INSERT par les valeurs des attributs d'un objet employé. 
    @Override
    ...
    //Méthode permettant de remplacer les parenthèse dans la requête SQL UPDATE par les valeurs des attributs d'un objet employé. 
    @Override
    protected void entityBindValuesWithId(PreparedStatement stmt, Employee entity) throws SQLException {
