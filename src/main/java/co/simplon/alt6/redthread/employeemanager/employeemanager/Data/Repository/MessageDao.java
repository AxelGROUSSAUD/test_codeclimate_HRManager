...
public class MessageDao extends AbstractDao<Message> implements MessageDaoI{
    ...
    public MessageDao() {
        super("INSERT INTO notification(idEmployee, idStatus, date, author, title, message) VALUES (?, ?, ?, ?, ?, ?)"
        , "SELECT * FROM notification"
        , "SELECT * FROM notification WHERE id = ?"
        , "UPDATE notification SET idEmployee = ?, idStatus = ?, date = ?, author = ?, title = ?, message = ? WHERE id = ?"
