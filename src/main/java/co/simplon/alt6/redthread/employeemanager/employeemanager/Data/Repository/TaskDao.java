...
public class TaskDao extends AbstractDao<Task> implements TaskDaoI {
  ...
  public TaskDao() {
    super(
        "INSERT INTO task (idEmployee, idProject, idStatus, name, description, startDate, endDate,"
            + " estimatedDuration, progress, timePassed, isAssign, comment) VALUES (?, ?, ?, ?, ?,"
            + " ?, ?, ?, ?, ?, ?, ?)",
        "SELECT * FROM task",
        "SELECT * FROM task WHERE id = ?",
        "UPDATE task SET idEmployee = ?, idProject = ?, idStatus = ?, name = ?, description = ?,"
            + " startDate = ?, endDate = ?, estimatedDuration = ?, progress = ?, timePassed = ?,"
            + " isAssign = ?, comment = ? WHERE id = ?",
